...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Edit Layanan: {{ $layanan->nama_layanan }}</h1>
...
<div class="card shadow mb-4">
    <div class="card-body">
        <form action="{{ route('admin.cms.layanan.update', $layanan->id) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="form-group">
                <label>Nama Layanan <span class="text-danger">*</span></label>
                <input type="text" name="nama_layanan" id="nama_layanan" class="form-control" required maxlength="100" value="{{ old('nama_layanan', $layanan->nama_layanan) }}">
            </div>

            <div class="form-group">
                <label>Deskripsi <span class="text-danger">*</span></label>
                <textarea name="deskripsi" id="deskripsi" class="form-control" rows="3" required>{{ old('deskripsi', $layanan->deskripsi) }}</textarea>
            </div>

            <div class="form-group">
                <label>Pilih Icon (FontAwesome 5) <span class="text-danger">*</span></label>
                <input type="hidden" name="icon_class" id="icon_class" value="{{ old('icon_class', $layanan->icon_class) }}" required>
            ...
            <div class="form-group">
                <label class="font-weight-bold">Route Aktif</label>
                <div class="alert alert-light border p-2 mb-0">
                    <code class="text-primary">{{ $layanan->route_name }}</code>
                    <small class="d-block text-muted mt-1">
                        <i class="fas fa-info-circle mr-1"></i>
                        Route tidak berubah saat edit
                    </small>
                </div>
            </div>
            ...
            <div class="form-group">
                <label>URL</label>
                <input type="url" name="url" id="url" class="form-control" placeholder="https://..." value="{{ old('url', $layanan->url) }}">
            ...
            <div class="form-row">
                <div class="form-group col-md-6">
                    <label>Urutan</label>
                    <input type="number" name="urutan" id="urutan" class="form-control" min="0" value="{{ old('urutan', $layanan->urutan) }}">
                </div>
                <div class="form-group col-md-6">
                    <label class="d-block">Status Aktif</label>
                    <div class="custom-control custom-switch mt-2">
                        <input type="checkbox" class="custom-control-input" id="is_active" name="is_active" {{ old('is_active', $layanan->is_active) ? 'checked' : '' }}>
