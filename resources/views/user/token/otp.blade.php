...
@section('title', 'Verifikasi OTP Booking Futsal')
...
<div class="container py-5 mt-4">
    <div class="row justify-content-center">
        <div class="col-md-6 col-lg-5">
            ...
            @php
                /* Masking nomor HP: sembunyikan semua kecuali 4 digit terakhir */
                $noHp      = $bookingFutsal->no_hp;
                ...
                $blockDetikSisa = $isBlocked
                    ? max(0, (int) now()->diffInSeconds($bookingFutsal->otp_blocked_until))
                    : 0;

                /* Sisa detik untuk countdown resend */
                $cooldownSisa = 0;
                ...
                /* Sisa detik OTP expired */
                $otpSisaDetik = 0;
                if ($bookingFutsal->otp_expired_at && now()->lt($bookingFutsal->otp_expired_at)) {
                    $otpSisaDetik = max(0, (int) now()->diffInSeconds($bookingFutsal->otp_expired_at));
                }
            @endphp
            ...
            <div class="otp-card card mb-4">
                ...
                <div class="card-body p-4">
                    {{-- Flash messages (dari publik layout sudah render di luar card, ini untuk inside card) --}}
                    @if(session('error'))
                    ...
                    {{-- BLOKIR BANNER (Poin 9: sembunyikan form jika diblokir) --}}
                    @if($isBlocked)
                    ...
                    @else
                        {{-- Form Input OTP --}}
                        <form id="otp-form" method="POST" action="{{ route('user.token.otp.verify', $token) }}" novalidate>
                            @csrf
                    ...
                    <div class="text-center">
                        ...
                        <form method="POST" action="{{ route('user.token.otp.resend', $token) }}" id="resend-form">
                            @csrf
                            @if($isBlocked)
...
(function () {
    ...
    /* =====================================================
       2. OTP validity countdown timer
    ===================================================== */
    ...
    if (blockEl) {
        ...
        const blockInterval = setInterval(function () {
            ...
            if (blockSisa <= 0) {
                ...
                // Auto refresh agar halaman diperbarui
    ...
    /* =====================================================
       5. Prevent double-submit on verify
    ===================================================== */
    if (form) {
