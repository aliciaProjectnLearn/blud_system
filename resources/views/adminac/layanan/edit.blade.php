...
@section('title', 'Edit Layanan AC')

@section('content')
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Edit Layanan AC</h1>
...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Form Edit Layanan AC</h6>
    </div>
    <div class="card-body">
        @if ($errors->any())
        ...
        <form action="{{ route('admin.ac.layanan.update', $layanan->id) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="col-sm-9">
                    <select name="kategori_id" class="form-control @error('kategori_id') is-invalid @enderror" required>
                        ...
                        @foreach($kategoris as $k)
                            <option value="{{ $k->id }}" {{ (old('kategori_id') ?? $layanan->kategori_id) == $k->id ? 'selected' : '' }}>{{ $k->nama }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-9">
                    <input type="text" name="kapasitas_ac" class="form-control @error('kapasitas_ac') is-invalid @enderror" value="{{ old('kapasitas_ac') ?? $layanan->kapasitas_ac }}">
                </div>
            </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-9">
                    <input type="number" name="harga_jasa" class="form-control @error('harga_jasa') is-invalid @enderror" value="{{ old('harga_jasa') ?? $layanan->harga_jasa }}" min="0" required>
                </div>
            </div>
