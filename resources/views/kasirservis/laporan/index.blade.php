...
@section('title', 'Laporan Kasir Servis Kendaraan')
...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 bg-white d-flex flex-column flex-sm-row align-items-start align-items-sm-center justify-content-between">
            ...
            <div class="d-flex flex-wrap gap-2">
                <a href="{{ route('kasir.laporan.export.pdf', request()->all()) }}" class="btn btn-danger btn-sm shadow-sm mr-2 mb-2 mb-sm-0" target="_blank">
                    <i class="fas fa-file-pdf fa-sm"></i> Export PDF
                </a>
                <a href="{{ route('kasir.laporan.export.excel', request()->all()) }}" class="btn btn-success btn-sm shadow-sm mb-2 mb-sm-0">
                    <i class="fas fa-file-excel fa-sm"></i> Export Excel
                </a>
            </div>
        </div>
        <div class="card-body">
            <form action="{{ route('kasir.laporan.index') }}" method="GET" class="row">
                ...
                <div class="col-md-2 mb-3 d-flex flex-column justify-content-end">
                    <div class="d-flex w-100 mt-2 mt-md-0">
                        ...
                        @if(request()->anyFilled(['tanggal_dari', 'tanggal_sampai', 'metode_pembayaran', 'status']))
                            <a href="{{ route('kasir.laporan.index') }}" class="btn btn-light shadow-sm ml-2" title="Reset Filter">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body p-0">
                ...
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        @forelse ($laporan as $index => $item)
                            <tr>
                                <td class="px-4 py-3 align-middle">{{ $laporan->firstItem() + $index }}</td>
                                <td class="px-4 py-3 align-middle">
                                    <div class="font-weight-bold text-primary">{{ $item->bookingServis->kode_booking ?? '-' }}</div>
                                    <div class="small text-muted">{{ $item->tanggal_bayar ? $item->tanggal_bayar->format('d/m/Y H:i') : '-' }}</div>
                                </td>
                                <td class="px-4 py-3 align-middle">
                                    <div class="font-weight-bold text-gray-800">{{ $item->bookingServis->pelanggan->name ?? '-' }}</div>
                                    <div class="small text-muted">{{ $item->bookingServis->nomor_plat ?? '-' }}</div>
                                </td>
                                <td class="px-4 py-3 align-middle">
                                ...
                                <td class="px-4 py-3 align-middle text-center">
                                    ...
                                    @endphp
                                    <span class="badge badge-{{ $badgeClass }} px-3 py-2 text-uppercase">
            ...
            <div class="d-md-none">
                @forelse ($laporan as $index => $item)
                    <div class="card border-bottom-0 border-left-0 border-right-0 rounded-0 p-3 {{ $loop->last ? '' : 'border-bottom' }}">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <div>
                                <span class="badge badge-primary mr-1">#{{ $laporan->firstItem() + $index }}</span>
                                <span class="font-weight-bold text-primary">{{ $item->bookingServis->kode_booking ?? '-' }}</span>
                            </div>
                            <div class="small text-muted">{{ $item->tanggal_bayar ? $item->tanggal_bayar->format('d/m/Y H:i') : '-' }}</div>
                        </div>
                        ...
                        <div class="mb-2">
                            <div class="font-weight-bold text-gray-800">
                                <i class="fas fa-user text-gray-400 mr-1"></i> {{ $item->bookingServis->pelanggan->name ?? '-' }} 
                                <span class="text-muted font-weight-normal">({{ $item->bookingServis->nomor_plat ?? '-' }})</span>
                            </div>
                            <div class="small text-muted mt-1">
                                <i class="fas fa-tools text-gray-400 mr-1"></i> {{ $item->bookingServis->layananServis->nama_layanan ?? '-' }}
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-between align-items-end mt-3 border-top pt-2">
                            <div>
                                ...
                                <div class="font-weight-bold text-gray-900">Rp {{ number_format($item->total_biaya, 0, ',', '.') }}</div>
                            </div>
                            <div class="text-right">
                                @if($item->tipe_pembayaran)
                                    <span class="badge badge-light border shadow-sm text-uppercase d-block mb-1">{{ $item->tipe_pembayaran }}</span>
                                @endif
